<template>
    ...
    <div class="list-container">
        <div class="list-container__item" v-for="iphone in iphones">
            <img :src="iphone.image.asset.url" class="item__image">
            <div class="item__title">{{ iphone.title }}</div>
            <div v-if="iphone.available === true" class="item__available--true">Available</div>
            <div v-else class="item__available--false">Sold out</div>
		    <div class="item__price">{{ iphone.price }} NOK</div>
...
export default {
	...
	async created() {
        ...
        this.iphones = this.products.filter(product => product.category[0].title === 'iPhone');

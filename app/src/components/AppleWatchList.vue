<template>
    ...
    <div class="list-container">
        <div class="list-container__item" v-for="watch in watches">
            <img :src="watch.image.asset.url" class="item__image">
            <div class="item__title">{{ watch.title }}</div>
            <div v-if="watch.available === true" class="item__available--true">Available</div>
            <div v-else class="item__available--false">Sold out</div>
		    <div class="item__price">{{ watch.price }} NOK</div>
...
export default {
	...
	async created() {
        ...
        this.watches = this.products.filter(product => product.category[0].title === 'Apple Watch');

<template>
    ...
    <div class="list-container">
        <div class="list-container__item" v-for="mac in macs">
            <img :src="mac.image.asset.url" class="item__image">
            <div class="item__title">{{ mac.title }}</div>
            <div v-if="mac.available === true" class="item__available--true">Available</div>
            <div v-else class="item__available--false">Sold out</div>
		    <div class="item__price">{{ mac.price }} NOK</div>
...
export default {
	...
	async created() {
        ...
        this.macs = this.products.filter(product => product.category[0].title === 'Mac');
